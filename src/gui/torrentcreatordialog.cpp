...
#ifdef QBT_USES_LIBTORRENT2
    m_threadPool.start([this, path, pieceSize = getPieceSize(), format = getTorrentFormat()]() {
        const int count = BitTorrent::TorrentCreator::calculateTotalPieces(path, pieceSize, format);
        QMetaObject::invokeMethod(this, &TorrentCreatorDialog::piecesCountCalculated, count);
    });
#else
    const bool isAlignmentOptimized = m_ui->checkOptimizeAlignment->isChecked();
    m_threadPool.start([this, path, pieceSize = getPieceSize(), isAlignmentOptimized, padded = getPaddedFileSizeLimit()]() {
